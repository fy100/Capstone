...
d$demo_q4 #test
####################### Visualization 1 #############################
# Set the order of sns_q7 levels
...
# Scatter plot for lone_score vs. sns_q7
ggplot(d, aes(x = sns_q7, y = lone_score, color = sns_q7)) +
  geom_jitter(alpha = 0.7, width = 0.3) +
  geom_smooth(method = "lm", se = FALSE, aes(group = 1), color = "#FF0000", size = 0.7) +  
  scale_color_manual(values = color_palette) + 
  labs(x = "Daily time spent on social media", y = "Loneliness indicators", title = "Correlation between Loneliness and Social Media Daily Usage ")

# Scatter plot for phq_score vs. sns_q7
ggplot(d, aes(x = sns_q7, y = phq_score, color = sns_q7)) +
  geom_jitter(alpha = 0.7, width = 0.3) +
  geom_smooth(method = "lm", se = FALSE, aes(group = 1), color = "#FF0000", size = 0.7) +  
  scale_color_manual(values = color_palette) + 
  labs(x = "Daily time spent on social media", y = "Depression indicators", title = "Correlation between Depression and Social Media Daily Usage ")

# Scatter plot for gad_score vs. sns_q7
ggplot(d, aes(x = sns_q7, y = gad_score, color = sns_q7)) +
  geom_jitter(alpha = 0.7, width = 0.3) +
  geom_smooth(method = "lm", se = FALSE, aes(group = 1), color = "#FF0000", size = 0.7) +  
  scale_color_manual(values = color_palette) + 
  labs(x = "Daily time spent on social media", y = "Anxiety indicators", title = "Correlation between Anxiety and Social Media Daily Usage ")

# Scatter plot for sleep_score vs. sns_q7
ggplot(d, aes(x = sns_q7, y = sleep_score, color = sns_q7)) +
  geom_jitter(alpha = 0.7, width = 0.3) +
  geom_smooth(method = "lm", se = FALSE, aes(group = 1), color = "#FF0000", size = 0.7) +  
  scale_color_manual(values = color_palette) + 
  labs(x = "Daily time spent on social media", y = "Sleep quality indicators", title = "Correlation between Sleep Quality and Social Media Daily Usage ")

####################### Visualization 2 ############################# 
# Grouped Bar Plot
ggplot(d, aes(x = sns_q7, y = total_score, fill = demo_q2)) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge") +
  labs(x = "Daily time spent on social media", y = "Mean Total Score", 
       fill = "Demo_q2") +
  ggtitle("Mean Total Score by SNS Usage and Demo_q2") +
  theme_minimal()
...
ggplot(d, aes(x = sns_q7, y = total_score, color = demo_q2)) +
  ...
  labs(x = "Daily time spent on social media", y = "Mean Total Score",
       color = "Demo_q2") +
  ggtitle("Interaction Plot: Total Score by SNS Usage and Demo_q2") +
  theme_minimal()
...
ggplot(correlation_data, aes(x = variable, y = sns, fill = correlation)) +
  geom_tile() +
  scale_fill_gradient2(high = "purple", low = "white", midpoint = 0,
...
d$sns_opp <- rowSums(d[, c("sns_q14", "sns_q15", "sns_q16", "sns_q17", "sns_q18", "sns_q19")])
...
# Linear regression between sns_use and total_score
lm_use <- lm(total_score ~ sns_use, data = d)

# Scatter plot and trend line for sns_use and total_score
scatterplot_sns_use <- ggplot(d, aes(x = sns_use, y = total_score)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "sns_use", y = "total_score", title = "Linear Regression: sns_use vs. total_score")

# Check assumptions for linear regression between sns_use and total_score
# scatterplotMatrix(~ sns_use + total_score, data = d)

## Regression Assumptions ####
# Residual plot
plot(resid(lm_use)~fitted(lm_use), main = "Residual Plot for SNS use vs total score",
     ylab = "Residuals", xlab = "Fitted Values")
abline(h = 0)

# Q-Q plot
qqnorm(resid(lm_use))
qqline(resid(lm_use), col = "red")
...
# Check assumptions for linear regression between sns_opp and total_score
#scatterplotMatrix(~ sns_opp + total_score, data = d)
...
######################### F test and ANOVA ###############################
# Fit the full model with sns_q7
full_model <- lm(total_score ~ sns_q7, data = d)

# Fit the reduced model with only intercept
reduced_model <- lm(total_score ~ 1, data = d)
...
#  regression between sns_use and total_score using LOESS
loess_use <- loess(total_score ~ sns_use, data = d)

# Scatter plot and LOESS curve for sns_use and total_score
scatterplot_sns_use_loess <- ggplot(d, aes(x = sns_use, y = total_score)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, color = "#FF0000") +
  labs(x = "Daily Time Spent on Social Media", y = "Psychological distress indicators", title = "Correlation between Overall Psychological Distress and Daily Time Spent on Social Media")
...
scatterplot_sns_opp_loess <- ggplot(d, aes(x = sns_opp, y = total_score)) +
  ...
  labs(x = "sns_opp", y = "total_score", title = "Nonparametric Regression: sns_opp vs. total_score")
...
quadratic_model <- lm(total_score ~ sns_opp + I(sns_opp^2), data = d)

# Summary of quadratic regression model
summary(quadratic_model)

# Predicted values and residuals
predicted <- fitted(quadratic_model)
...
# Scatter plot with quadratic regression curve
scatterplot_sns_opp_quadratic <- ggplot(d, aes(x = sns_opp, y = total_score)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), se = FALSE, color = "blue") +
  labs(x = "sns_opp", y = "total_score", title = "Quadratic Regression: sns_opp vs. total_score")
...
d$sns_q6 <- as.factor(d$sns_q6)
d$sns_q7 <- as.factor(d$sns_q7)
d$sns_q9 <- as.factor(d$sns_q9)
d$sns_q14 <- as.factor(d$sns_q14)
d$sns_q15 <- as.factor(d$sns_q15)
...
summary(mlm)
mlm <- lm(cbind(total_score) ~ sns_q5 + sns_q6 + sns_q7 + sns_q9 + sns_q14 + sns_q15 +sns_q16 + sns_q17 + sns_q18 + sns_q19, data = d)
summary(mlm)
# MLM with picked demo predictor
mlm_demo <- lm(cbind(total_score) ~ demo_q2 + demo_q10 + demo_q12, data = d) 
summary(mlm_demo)

# overall MLM -- best
mlm2 <- lm(cbind(total_score) ~ sns_q5 + sns_q7 + sns_q13 + sns_q16 + sns_q14 + sns_q17 + sns_q18 + sns_q19 + demo_q2 + demo_q10 + demo_q12, data = d)
summary(mlm2)
...
mlm2 <- lm(cbind(total_score) ~ sns_q7 + sns_q13 + sns_q14 + sns_q17 + sns_q18 + sns_q19 + demo_q2 + demo_q10 + demo_q12, data = d)

# Summary of the multiple linear regression model
summary(mlm2)

############## Linear Regression After Filtering Predictor ################
...
# Check assumptions for linear regression between sns_use and total_score
scatterplotMatrix(~ sns_q7 + total_score, data = d)

# Calculate SNS Opinions (new col)
d$sns_opp_new <- rowSums(d[, c("sns_q14", "sns_q17", "sns_q18", "sns_q19")])

# Linear regression between sns_opp and total_score
lm_opp_new <- lm(total_score ~ sns_opp_new, data = d)

# Scatter plot and trend line for sns_opp and total_score
scatterplot_sns_opp_new <- ggplot(d, aes(x = sns_opp_new, y = total_score)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "sns_opp_new", y = "total_score", title = "Linear Regression: sns_opp_new vs. total_score")

# Check assumptions for linear regression between sns_opp and total_score
scatterplotMatrix(~ sns_opp_new + total_score, data = d)
